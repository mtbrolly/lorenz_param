...
if local:
    MODEL_DIR = (
        f"models/l96_MDN_local_MSD_c{c:.0f}_nc{n_components:.0f}/")
else:
    MODEL_DIR = ("models/l96_MDN_nonlocal_MSD_"
                 + f"c{c:.0f}_nc{n_components:.0f}_newtfp/")
...
if local:
    X = X_n.flatten()[::sample_rate, None]
    Y = M_n.flatten()[::sample_rate, None]
else:
    X = X_n[::sample_rate]
    Y = M_n[::sample_rate]
...
if local:
    model = load_independent_MDN(activation_fn=activation_fn,
                                 n_hidden_layers=n_hidden_layers,
                                 n_neurons_per_layer=n_neurons_per_layer,
                                 n_components=n_components,
                                 multiple_gpu=multiple_gpu)
else:
    model = load_multivariate_MDN(output_size=Y_.shape[-1],
